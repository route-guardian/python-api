...
def getLatLong(data):
    ...
    response = requests.get("https://route.api.here.com/routing/7.2/calculateroute.json?app_id=" + APPID + "&app_code=" + APPCODE + "&waypoint0=geo!"+ str(data["startPoint"]["lat"]) +"," + str(data["startPoint"]["long"]) + "&waypoint1=geo!"+ str(data["endPoint"]["lat"]) +"," + str(data["endPoint"]["long"]) + "&mode=fastest;pedestrian;traffic:disabled&avoidareas=" + stringAvoidArreas).json()
